 ...
 */
final class TemplateAsm extends TemplateAsmConst implements Opcodes {
	 ...
	 * 
	 * @param genCls
	 *            internal name for class to be generated
	 * @param victimCls
	 *            internal name for victim class
	 * @param writerCls
	 *            internal name for writer class
	 * @param ranges
	 *            literal data descriptors
	 * @param invokers
	 *            substitute method names
	 * @return generated byte-code
	 */
	final static byte[] dump(String genCls, String victimCls,
			String writerCls, Range[] ranges, String[] invokers) {
		...
		// | call _vic.getOut
		mv.visitMethodInsn(INVOKEVIRTUAL, victimCls, GET_OUT, PARAMETERLESS
				+ l(writerCls));
		// save writer
		...
		// 2
		mv.visitLocalVariable("_out", l(writerCls), null, labelVicFree,
				labelEnd, LOCAL_OUT);
		mv.visitMaxs(4, 3);
		mv.visitEnd();
		cw.visitEnd();
	 ...
	 */
	private final static void pushLiteral(MethodVisitor mv, Range range,
			String writerCls) {
		int len = range.len;
		if (len == 0) {
			return;
		...
		pushInt(mv, len);
		mv.visitMethodInsn(INVOKEVIRTUAL, writerCls, WRITE, WRITE_PARAMS);
	}
}